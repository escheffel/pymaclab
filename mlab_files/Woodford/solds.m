...
% ****************************************************************
% The starting point now is the output of reds.m, i.e., the
% already reduced system. This system is represented by the
% current B and C (recall that A must have been reduced to zeros
% all over, except for a southeastern identity block). We have
% also kept track of L, the reordering matrix.
%
...
%
% where Bse is the southeastern block of B.
%
...
[LE1 MU1] = eig(B(NF+1:NY, NF+1:NY)');
LE1 = LE1';
MU1 = MU1';

[LE2 MU2] = eig(B(NF+1:NY, NF+1:NY)', 'nobalance');
LE2 = LE2';
MU2 = MU2';

if sum(sum(abs(LE1*B(NF+1:NY, NF+1:NY) - MU1*LE1))) > ...
                sum(sum(abs(LE2*B(NF+1:NY, NF+1:NY) - MU2*LE2)))
        LE = LE2;
        MU = MU2;
        else
        LE = LE1;
...
% THETA(i, :), each of length cols(C) = cols(Cs).
%
% But now we have:
%
...
if NUE > 0
        ...
        if NK > 0
                ...
                B(:, NF+NUE+1:NY) = B3+B2*HK;
                end

        HX = LE(:, 1:NUE)\THETA;
        C = C+B2*HX;

        B(:, NF+1:NF+NUE) = zeros(size(B2));
        B(NF+1:NF+NUE, NF+1:NF+NUE) = eye(NUE);

        if NK > 0
                B(NF+1:NF+NUE, NF+NUE+1:NY) = -HK;
                end
