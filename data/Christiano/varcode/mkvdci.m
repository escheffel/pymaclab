function [cil,ciu,varimpneg,varimppos,varz]=mkvdci(beta,a0,nlags,errshk,nstep,ndraws,nobs,pctg,resid,sigma,numtries,mkmatrix,mkstart,varargin)
% MKIVDCI calculates confidence intervals for varidance decompositons.
% Syntax [cil,ciu,varimpneg,varimppos,varz]=mkvdci(beta,a0,hascon,ndraws,nobs,nstep,errshk,errshk,pctg,residsigma,numtries,mkmatrix,mkstart,p1,p2,...)
...
for icnt=1:ndraws;
   ...
   [mse msedecomp]=vardecomp(best,a0est,nlags,errshk,nstep);
   %if you don't have four dimensional arrays this will break.
        
   cnfdints(:,:,:,icnt)=msedecomp./mse;
...
[mse msedecomp]=vardecomp(beta',a0,nlags,errshk,nstep);

trdecomp = msedecomp./mse;

varz = (1/ndraws)*(sum(cnfdints.^2,4)- mean(cnfdints,4).^2);
varimppos = trdecomp + 2*sqrt(varz);
varimpneg = trdecomp - 2*sqrt(varz);
